...
const NewCollection = () => {
  const [getNewCollectionData, setgetNewCollectionData] = useState([]);

  useEffect(() => {
    fetch('http://localhost:8080/getNewCollection')
      .then((response) => response.json())
      .then((data) => setgetNewCollectionData(data.slice(0, 4)))
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', backgroundColor: '#ffffff' }}>
      ...
      <div className="w-2/3 mx-auto flex items-center">
        ...
        <span className="px-4 font-extrabold text-4xl text-red-500  ">Our New Collection</span>
        <div className="flex-grow border-t border-gray-300 dark:border-gray-700"></div>
      </div>
